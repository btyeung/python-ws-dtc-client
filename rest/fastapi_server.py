...
app = FastAPI(title="DTC REST API", debug=True)
...
async def handle_websocket_connection(websocket: WebSocket, subscription_type: SubscriptionDataType):
...
async def market_data(websocket: WebSocket):
    await handle_websocket_connection(websocket, SubscriptionDataType.MARKET_DATA)

@app.websocket(API.API_PREFIX + API.MARKET_DEPTH)
async def market_depth(websocket: WebSocket):
    await handle_websocket_connection(websocket, SubscriptionDataType.MARKET_DEPTH)
...
async def historical_account_balance(
    ...
    start_date_time: Optional[str] = Query(None, alias=START_DATE_TIME)
):
    request_id = rest_server.get_next_request_id()
    response = dtc_client.request_response(
        request_id,
...
async def historical_order_fills(
    trade_account: str = Query(..., alias=TRADE_ACCOUNT),
    number_of_days: Optional[int] = Query(None, alias=NUMBER_OF_DAYS),
    start_date_time: Optional[str] = Query(None, alias=START_DATE_TIME)
):
...
class RESTServer:
    ...
    def start(self, _dtc_client, _rest_port):
        logging.info('Starting FastAPI REST Server')
        global dtc_client
